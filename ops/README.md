...
Serves via nginx at `https://contextforge.local:8443/` (port 8080 redirects to 8443 — HTTP_PORT/HTTPS_PORT in `.env`, change if those are taken too). nginx proxies to the FastAPI `api` container; `api`'s entrypoint runs `alembic upgrade head` before starting uvicorn. A separate `worker` container (same image, `python -m app.worker`, no entrypoint/migration step) polls the `document-ingestion` queue and indexes uploaded documents into Qdrant.
...
kubectl apply -f ops/k8s/02e-qdrant.yaml
...
kubectl apply -f ops/k8s/08-worker.yaml
```
