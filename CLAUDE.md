...
Routes don't contain logic. The flow for any endpoint:

```
app/api/routes/<resource>.py   — router.add_api_route(path, controller_fn, methods=[...], response_model=...)
app/controllers/<resource>_controller.py  — the actual handler: full FastAPI signature (params + Depends), DB calls, raises ApiError
...
4. Controller functions in `app/controllers/` with full `Depends(...)` signatures, returning `SuccessResponse[T]` / `PaginatedResponse[T]`, raising `app/core/errors.py` helpers on failure.
5. Router in `app/api/routes/`, wired via `router.add_api_route(...)`.
6. Include the router in `app/api/v1.py`.
7. **Tests first** (see below) in `tests/test_<resource>.py`.
